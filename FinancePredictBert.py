...
reuters_BERT = pd.DataFrame(columns=['label', 'text'])[:sample_num]
reuters_BERT.label = reuters.Y[:sample_num]
reuters_BERT.text = reuters.news.apply(lambda x: ' '.join(x))[:sample_num]

# Tokenize and encode text data
tokenizer = BertTokenizer.from_pretrained('Z:/User/Documents/bert-base-uncased/vocab.txt', do_lower_case=True)
...
model = BertForSequenceClassification.from_pretrained(model_path, num_labels=clusters).to(device)
...
for epoch in range(epochs):
    ...
    output_path = "Bert_model_train_results.txt"
    ...
    # Compute metrics
    acc, mcc, cm, f1 = compute_metrics(all_preds, all_labels)
    output_path = "Bert_model_evaluation_results.txt"
    with open(output_path, 'a') as f:
...
# Compute metrics
acc, mcc, cm, f1 = compute_metrics(all_preds, all_labels)
output_path = "Bert_model_test_results.txt"
